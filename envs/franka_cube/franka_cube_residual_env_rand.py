...
@configclass
class FrankaCubeResidualRandEnvCfg(DirectRLEnvCfg):
    # env
    episode_length_s = 6.6666 #3.31 ~ 200 timesteps  # 8.3333 = 500 timesteps
    decimation = 2
    action_space = 8 # waypoint
    observation_space = 3*8+4*8+7 # last 3 wp + last 3 expert wp
    state_space = 0
    ...
    camera = CameraCfg(
        ...
        offset=CameraCfg.OffsetCfg(pos=(0.2, 0.0, -0.04), rot=(0.9239556994702721, 0.0, -0.3824994972760097, 0.0), convention="ros"),
        height=96,
        width=96,
        data_types=[
            "rgb",
...
class FrankaCubeResidualRandEnv(DirectRLEnv):
    ...
    cfg: FrankaCubeResidualRandEnvCfg

    def __init__(self, cfg: FrankaCubeResidualRandEnvCfg, render_mode: str | None = None, **kwargs):
    ...
    def step(self, ee_residual):
        ...
        self._camera.update(dt=self.dt) #TODO: check what this is doing
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        reseted_root_states[env_ids,:2] += self.scene.env_origins[env_ids,:2] + sample_uniform(-0.12, 0.12, (len(env_ids), 2), self.device)
        reseted_root_states[env_ids,2] += self.scene.env_origins[env_ids,2] + sample_uniform(0, 0.15, len(env_ids), self.device)
        # TODO: add changes in orientation
