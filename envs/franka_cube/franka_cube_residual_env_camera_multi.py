...
@configclass
class FrankaCubeResidualCamMultiEnvCfg(DirectRLEnvCfg):
    # env
    episode_length_s = 6.6666 #3.31 ~ 200 timesteps  # 8.3333 = 500 timesteps
    decimation = 2
    action_space = 8 # waypoint
    observation_space = 3*8+4*8+9272 # last 3 wp + last 3 expert wp
    state_space = 0
    ...
    camera = TiledCameraCfg(
        prim_path="/World/envs/env_.*/Robot/panda_hand/cam",
        offset=TiledCameraCfg.OffsetCfg(pos=(0.2, 0.0, -0.04), rot=(0.866158094405463, 0.0, -0.4997701026431024, 0.0), convention="ros"),
        height=96,
        width=96,
        data_types=[
            # "rgb",
        ...
        spawn=sim_utils.PinholeCameraCfg(
            focal_length=18.0, focus_distance=400.0, horizontal_aperture=20.955, clipping_range=(0.1, 20)
        ),
    )
    ...
    height_reward_scale = 2.5 # change to 1.5?
    ee_dist_reward_scale = 1.5 # from 0.5
    residual_penalty_scale = -0.5 # equivalent to tracking rew?


class FrankaCubeResidualCamMultiEnv(DirectRLEnv):
    ...
    cfg: FrankaCubeResidualCamMultiEnvCfg

    def __init__(self, cfg: FrankaCubeResidualCamMultiEnvCfg, render_mode: str | None = None, **kwargs):
        super().__init__(cfg, render_mode, **kwargs)


        self.demo_traj1 = self.save_traj_as_tensor("/home/shuosha/Desktop/IsaacLab/IsaacLab/source/standalone/workflows/rsl_rl/wp_traj1/wp_traj_cube_bar4.txt")
        self.demo_traj2 = self.save_traj_as_tensor("/home/shuosha/Desktop/IsaacLab/IsaacLab/source/standalone/workflows/rsl_rl/wp_traj1/wp_traj_cube_bar2.txt")
        self.demo_traj3 = self.save_traj_as_tensor("/home/shuosha/Desktop/IsaacLab/IsaacLab/source/standalone/workflows/rsl_rl/wp_traj1/wp_traj_cube_bar3.txt")

        self.demo_traj = torch.stack([self.demo_traj1, self.demo_traj2, self.demo_traj3], dim=1)
        self.demo_num_per_env = torch.zeros(self.num_envs, device=self.device, dtype=torch.long).unsqueeze(1)
        ...
        self.env_idx = torch.arange(self.num_envs, device=self.device).unsqueeze(1)
    ...
    def _pre_physics_step(self, ee_residual: torch.Tensor):
        ...
        self.ee_residual = self.cfg.alpha * ee_residual.clone() # .clamp(-1.0, 1.0) # TODO: check if this is necessary
        # self.ee_residual = ee_residual.clone() # .clamp(-1.0, 1.0) # TODO: check if this is necessary

        selected_demos = self.demo_traj[self.env_idx, self.demo_num_per_env].squeeze(1)
        demo_action = selected_demos.gather(1, self.curr_col_idx)
        self.joint_pos = self.get_joint_pos_from_ee_pos(demo_action + self.ee_residual)
    ...
    def _reset_idx(self, env_ids: torch.Tensor | None):
        ...
        reseted_root_states[env_ids,0] += self.scene.env_origins[env_ids,0] + sample_uniform(-0.03, 0.05, len(env_ids), self.device) #x 
        reseted_root_states[env_ids,1] += self.scene.env_origins[env_ids,1] + sample_uniform(-0.1, 0.1, len(env_ids), self.device) #y
        reseted_root_states[env_ids,2] += self.scene.env_origins[env_ids,2] + sample_uniform(0, 0.05, len(env_ids), self.device) #z
        random_quats = sample_uniform(-1, 1, (len(env_ids), 4), self.device)
        random_quats /= torch.norm(random_quats, dim=-1, keepdim=True).clamp(min=1e-8)
        new_quats = reseted_root_states[env_ids,3:7] + random_quats
        new_quats /= torch.norm(new_quats, dim=-1, keepdim=True).clamp(min=1e-8)
        reseted_root_states[env_ids,3:7] = new_quats
        ...
        self.demo_num_per_env[env_ids] += 1
        self.demo_num_per_env[self.demo_num_per_env == 3] = 0
        # print("curr demo: ", self.demo_num)

        self.time_step_per_env[env_ids] = 0
        self.robot_wp[env_ids, :] = 0
        self.demo_wp[env_ids, :] = 0
        selected_demos = self.demo_traj[self.env_idx, self.demo_num_per_env].squeeze(1)
        self.demo_wp[env_ids,:8] = selected_demos[env_ids, :8]

    def _get_observations(self) -> dict:
        # self.save_camera_data()
        # rgb_input = self._camera.data.output["rgb"].permute(0,3,1,2) # From (B, H, W, 3) to (B, 3, H, W)
        # rgb_input = rgb_input.float() / 255.0 # int8 to float32            env_ids = self._robot._ALL_INDICES
        # rgb_input = 2*rgb_input-1

        depth_input = self._camera.data.output["distance_to_image_plane"].permute(0,3,1,2) # From (B, H, W, 1) to (B, 1, H, W)

        # Replace NaNs with zeros or handle appropriately
        depth_input = torch.nan_to_num(depth_input, nan=0.0)

        # # Compute min and max values
        # min_val = depth_input.min()
        # max_val = depth_input.max()

        # # Avoid division by zero if min and max are equal
        # if max_val - min_val > 0:
        #     depth_input = 2 * (depth_input - min_val) / (max_val - min_val) - 1
        # else:
        #     depth_input = torch.zeros_like(depth_input)  # Default to zero if all values are the same

        # assert not torch.isnan(depth_input).any(), "Depth input contains NaNs!"

        depth_input = depth_input.reshape(depth_input.shape[0], -1)
        depth_input = torch.clamp(depth_input, 0, 2)

        # cube_pos_b, cube_quat_b = subtract_frame_transforms(
        #     self._robot.data.body_state_w[:,8,:3], self._robot.data.body_state_w[:,8,3:7], self._cube.data.body_state_w[:,0,:3], self._cube.data.body_state_w[:,0,3:7]
        # )
        obs = torch.cat(
            (
                self.robot_wp,
        ...
        assert not torch.isnan(obs).any(), "Observations contain NaNs!"
    ...
    def save_camera_data(self):
        ...
        cam_data = convert_dict_to_backend(self._camera.data.output[0], backend="numpy")
        cam_info = self._camera.data.info[0]
        ...
        rep_output["trigger_outputs"] = {"on_time": self._camera.frame}
        rep_writer.write(rep_output)

    def save_traj_as_tensor(self, demo_path):
        ...
        demo_traj = torch.tensor(demo_traj_flatten, device=self.device).unsqueeze(0).expand(self.num_envs, -1)
        # print("demo traj shape: ", self.demo_traj.shape)
