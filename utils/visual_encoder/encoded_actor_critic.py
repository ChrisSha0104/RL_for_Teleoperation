...
class ActorCriticVisual(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if use_visual_encoder:
            mlp_input_dim_a = mlp_input_dim_a - (visual_idx[1] - visual_idx[0]) + encoder_output_dim
            mlp_input_dim_c = mlp_input_dim_c - (visual_idx[1] - visual_idx[0]) + encoder_output_dim
    ...
    def update_distribution(self, observations):
        ...
        if self.use_visual_encoder:
            visual_obs = observations[:,self.visual_idx[0]:self.visual_idx[1]].reshape(-1, 1, 96, 96)
            visual_obs = visual_obs.continguous() 
            visual_embedding = self.visual_encoder(visual_obs)
            observations = torch.cat((observations[:,:self.visual_idx[0]], visual_embedding), dim=-1)
    ...
    def evaluate(self, critic_observations, **kwargs):
        ...
        """
        if self.use_visual_encoder:
            visual_obs = critic_observations[:,self.visual_idx[0]:self.visual_idx[1]].reshape(-1, 1, 96, 96)
            visual_obs = visual_obs.continguous() 
            visual_embedding = self.visual_encoder(visual_obs)
            critic_observations = torch.cat((critic_observations[:,:self.visual_idx[0]], visual_embedding), dim=-1)
