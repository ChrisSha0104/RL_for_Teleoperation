    ...
    robot = ArticulationCfg(
        ...
        spawn=sim_utils.UsdFileCfg(
            ...
            rigid_props=sim_utils.RigidBodyPropertiesCfg(
                disable_gravity=True, # add gravity
                max_depenetration_velocity=5.0,
            ),
...
def run_simulator(sim: sim_utils.SimulationContext, scene: InteractiveScene):
    ...
    traj_path = os.path.join(current_dir, "wp_traj1","wp_traj_cube_bar4.txt")
    with open(traj_path, "r") as file:
        wp_traj = [line.strip() for line in file]

    # Define simulation stepping
    sim_dt = sim.get_physics_dt()
    count = 0
    # Simulation loop
    while simulation_app.is_running() and count < len(wp_traj):
        # reset
        if count == 0:
            # reset joint state
            ...
            ee_goal = torch.tensor(eval(wp_traj[count]), device=args_cli.device)
        else: 
            ee_goal_prev = ee_goal
            ee_goal = torch.tensor(eval(wp_traj[count]), device=args_cli.device)
            ik_commands = ee_goal[:-1] #(num_envs, 8)
            diff_ik_controller.set_command(ik_commands)
            finger_command = ee_goal[-1]
            ...
            if finger_command < 1: # 0 for open and 1 for closed
                finger_joints_des = torch.tensor([[0.04, 0.04]], device='cuda')
                robot.set_joint_position_target(finger_joints_des, joint_ids=[7,8])
            else:
                finger_joints_des = torch.tensor([[0.01, 0.01]], device='cuda')
                robot.set_joint_position_target(finger_joints_des, joint_ids=[7,8])

            # check residual
            # print("residual: ", torch.norm(ee_goal_prev[:-1] - ee_pose_w, dim=-1))
            ...
            # import pdb
            # pdb.set_trace()
            goal_marker.visualize(ik_commands.unsqueeze(0)[:,0:3] + scene.env_origins, ik_commands.unsqueeze(0)[:,3:7])
